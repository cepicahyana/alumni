	...
	function getData(){
		...
		$id	= $dataDB->id;
		 $action		 =	'<div class="btn-group" role="group"  >
								 <button onclick="hapus(`'.$dataDB->id.'`,`'.$dataDB->nama_kelas .'`)" type="button" class="btn bg-grey  btn-sm waves-effect waves-light ti-trash"> <i title="hapus" class="ri-delete-bin-7-fill"></i>   </button>
			...
			$row = array();
			$row[] = $no++;
			$row[] = $dataDB->id_tingkat;
			$row[] = $dataDB->nama_kelas;
			$row[] = $action;
