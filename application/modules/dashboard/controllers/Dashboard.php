...
class Dashboard extends MX_Controller
{
	...
	public function index()
	{
		...
		if ($ajax == "yes") {
			...
			$data['konten'] = "index";
			$data['profesi'] = $this->mdl->get_data('tr_pekerjaan')->result();
			$data['penghasilan'] = $this->mdl->get_data('tr_penghasilan')->result();
			$data['pendidikan'] = $this->mdl->get_data('tr_jp')->result();
			$data['goldar'] = $this->mdl->get_data('tr_goldar')->result();

			$alumni = $this->db->query("SELECT * FROM data_alumni
				WHERE sts='1'");
			$data['alumni'] = $alumni->num_rows();

			$registrasi = $this->db->query("SELECT * FROM data_alumni
				WHERE sts='0'");
			$data['registrasi'] = $registrasi->num_rows();
